...
export class Banana extends WeaponBase {
  ...
  private createBoomerang(angle: number, damage: number, speed: number, area: number): void {
    ...
    // 사거리 = 플레이어 신장 x 3 (displayHeight 미확정 시 폴백 80px), area는 업그레이드 배율로 유지
    const playerH = this.player.displayHeight || Banana.FALLBACK_PLAYER_HEIGHT;
    const distance = playerH * Banana.RANGE_HEIGHT_MULT * area;
